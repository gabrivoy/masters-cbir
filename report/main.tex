...
associados~\cite{smeulders2000cbir}. A ideia central: converter cada imagem em
um vetor numérico (um \emph{embedding}) que captura suas características visuais,
de modo que imagens parecidas fiquem próximas em um espaço vetorial e imagens
diferentes fiquem distantes. A busca por semelhança vira, então, uma busca por
vizinhos mais próximos nesse espaço.
...
visão como o CLIP~\cite{radford2021clip} produzem embeddings genéricos fortes, e
uma parte importante da pesquisa em CBIR é justamente medir o quão confiáveis
esses embeddings são para uma tarefa concreta. Este programa serve exatamente a
esse propósito: tornar visível e mensurável o comportamento de recuperação.
...
\emph{K-Nearest Neighbors} (KNN)~\cite{cover1967knn} sobre os vizinhos
recuperados a que classe a imagem consultada pertenceria, com um grau de
confiança.
...
\subsection{Fundamentos: PCA e redução de dimensionalidade}

Cada \emph{embedding} produzido pelo modelo é um vetor de alta dimensão (no caso
do OpenCLIP ViT-B/32, $d = 512$ dimensões). Esse espaço é o que permite medir
semelhança com precisão, mas é \textbf{impossível de visualizar diretamente}:
não há como desenhar um ponto em 512 eixos. Para inspecionar o espaço a olho nu
precisamos projetá-lo em 2 ou 3 dimensões, aceitando que \emph{parte da
informação será perdida} nessa compressão. É um compromisso consciente: trocamos
fidelidade por interpretabilidade visual.

A Análise de Componentes Principais (PCA)~\cite{jolliffe2002pca} faz essa
redução buscando as direções de \emph{maior variância} dos dados. Dada uma
matriz $X \in \mathbb{R}^{n \times d}$ com $n$ embeddings centrados (média zero),
a PCA calcula a matriz de covariância

\[
  C = \frac{1}{n} X^\top X \in \mathbb{R}^{d \times d},
\]

e resolve seu problema de autovalores $C v_i = \lambda_i v_i$. Os autovetores
$v_i$ (componentes principais) são ortogonais e ordenados pelos autovalores
$\lambda_i$, que medem quanta variância cada direção captura. Para visualizar em
$k$ dimensões ($k = 2$ ou $3$), tomamos os $k$ autovetores de maior autovalor,
formando $W_k = [\,v_1\ \cdots\ v_k\,] \in \mathbb{R}^{d \times k}$, e projetamos

\[
  Z = X\,W_k \in \mathbb{R}^{n \times k}.
\]

A fração de variância preservada é $\sum_{i=1}^{k} \lambda_i \big/
\sum_{j=1}^{d} \lambda_j$; o relatório da interface mostra esse valor para deixar
explícito o quanto foi perdido. Por ser uma transformação \emph{linear} (a
matriz $W_k$ ajustada na galeria), a PCA projeta uma imagem de consulta nova no
mesmo espaço com a mesma multiplicação $z = x\,W_k$, o que é a propriedade
essencial para posicionar a consulta em relação aos aglomerados existentes.

A Figura~\ref{fig:pca} ilustra a ideia geométrica em duas dimensões: a primeira
componente principal ($v_1$) aponta na direção de maior espalhamento dos dados,
e a segunda ($v_2$), ortogonal, na direção de maior variância restante. Projetar
sobre $v_1$ preserva o máximo de informação em um único eixo.

\begin{figure}[H]
  \centering
  \begin{tikzpicture}[scale=1.0]
    % nuvem de pontos alongada ao longo da diagonal (maior variância em v1)
    \foreach \x/\y in {
      -2.6/-1.3, -2.1/-1.2, -1.9/-0.8, -1.5/-0.9, -1.2/-0.5, -1.0/-0.7,
      -0.7/-0.2, -0.5/-0.5, -0.3/0.0, -0.1/-0.2, 0.1/0.2, 0.3/-0.1,
      0.5/0.4, 0.7/0.1, 1.0/0.5, 1.2/0.3, 1.5/0.9, 1.8/0.6,
      2.0/1.1, 2.4/0.9, -1.6/-0.4, -0.9/0.2, 0.0/-0.6, 0.9/-0.1,
      1.6/0.4, -2.2/-0.7, 0.4/0.7, 1.3/0.8, -0.4/-0.9, 2.1/0.6}
      \fill[gray!55] (\x,\y) circle (2.1pt);

    % eixos leves
    \draw[->,gray!40] (-3.2,0) -- (3.2,0);
    \draw[->,gray!40] (0,-2.0) -- (0,2.0);

    % v1: direção de maior variância (diagonal)
    \draw[-{Stealth[length=3mm]},very thick,black]
      (0,0) -- (2.4,1.15) node[above right,black] {$v_1$ (maior variância)};
    % v2: ortogonal, menor variância
    \draw[-{Stealth[length=3mm]},very thick,black]
      (0,0) -- (-0.65,1.35) node[left,black] {$v_2$};
  \end{tikzpicture}
  \caption{Intuição do PCA em 2D. As componentes principais $v_1$ e $v_2$ são
  ortogonais e apontam nas direções de maior variância dos dados; reduzir a
  dimensionalidade equivale a manter as primeiras dessas direções.}
  \label{fig:pca}
\end{figure}

\subsection{Por que PCA (e não t-SNE/UMAP)}

A projeção usa Análise de Componentes Principais
(PCA)~\cite{jolliffe2002pca}, uma redução linear de dimensionalidade. A
alternativa seriam métodos não lineares como o t-SNE~\cite{vandermaaten2008tsne}
ou o UMAP~\cite{mcinnes2018umap}, mas eles não oferecem uma transformação exata
para projetar um ponto novo (a consulta) no mesmo espaço da galeria já ajustada.
A Tabela~\ref{tab:pca} resume a comparação.
...
\begin{itemize}
  \item \textbf{Python 3.13} com \textbf{uv}: linguagem do projeto e gerenciador
  de dependências/ambientes (rápido, com \emph{lockfile} reproduzível).
  \item \textbf{OpenCLIP}~\cite{ilharco2021openclip}: implementação aberta do
  modelo CLIP, que converte uma imagem em um vetor (\emph{embedding}) que captura
  seu conteúdo visual.
  \item \textbf{Milvus}~\cite{wang2021milvus}: banco de dados vetorial,
  especializado em armazenar \emph{embeddings} e fazer busca por vizinhos mais
  próximos em larga escala.
  \item \textbf{scikit-learn (PCA)}~\cite{pedregosa2011scikit}: biblioteca de
  \emph{machine learning}; usamos o PCA para reduzir os vetores a 2D/3D
  preservando as direções de maior variância.
  \item \textbf{FastAPI}: \emph{framework} web para APIs em Python, com validação
  e documentação automáticas; expõe o \emph{backend} via HTTP.
  \item \textbf{Streamlit} com \textbf{Plotly}: Streamlit constrói a interface web
  em Python puro; Plotly desenha os gráficos de dispersão 2D/3D interativos.
  \item \textbf{Pydantic v2}: valida e tipa os dados que trafegam entre as
  camadas, garantindo que todas concordem sobre o formato.
  \item \textbf{ruff}, \textbf{mypy}, \textbf{pytest}: \emph{linter}/formatador,
  verificador de tipos estático e \emph{framework} de testes, respectivamente.
  \item \textbf{Docker Compose}: orquestra os serviços (Milvus, API, interface)
  em contêineres, para subir tudo com um comando.
\end{itemize}
...
A suíte tem \textbf{33 testes} e foi desenhada para ser rápida e determinística:
o \emph{backend} é puro e testado sem Milvus nem \emph{torch}, e a API é testada
com um serviço falso (dublê), o que dispensa qualquer dependência externa e
ainda permite verificar a garantia de consistência de modelo (resposta
\texttt{409}). Os testes rodam em poucos segundos.

\begin{table}[H]
\centering
\begin{tabular}{@{}p{2.6cm}p{10cm}r@{}}
\toprule
\textbf{Arquivo} & \textbf{O que cobre} & \textbf{Nº} \\
\midrule
\texttt{test\_knn} & Voto KNN: conjunto vazio, unanimidade, maioria vs.\ voto ponderado, corte por \texttt{k}, similaridade negativa sem subtrair evidência, desempate determinístico & 7 \\
\texttt{test\_manifest} & Contrato de dados: rejeição de \texttt{item\_id} duplicado, campos obrigatórios, filtros por split/benchmark, amostragem determinística por classe, recorte de bbox com clip nas bordas & 7 \\
\texttt{test\_projection} & PCA: dimensionalidade pedida, \texttt{transform} reproduz a galeria, vetor único, degradação graciosa, galeria vazia, determinismo & 6 \\
\texttt{test\_api} & Contrato HTTP: \texttt{health}, coleções com seu modelo, projeção e 404, consulta feliz com predição, upload inválido rejeitado & 6 \\
\texttt{test\_models} & Modelos Pydantic: limites de confiança, \texttt{rank} positivo, campos \texttt{model\_*}, \emph{round-trip} JSON & 4 \\
\texttt{test\_scatter} & Construção do gráfico Plotly 2D/3D no \emph{frontend}, incluindo consulta e vizinhos destacados & 3 \\
\bottomrule
\end{tabular}
\caption{Cobertura da suíte de testes (33 no total).}
\end{table}

\noindent Trecho ilustrativo (o voto ponderado por similaridade pode inverter o
vencedor em relação à maioria simples):
...
\noindent Execução completa (as três verificações devem ficar verdes):
...
\texttt{ruff} / \texttt{mypy} / \texttt{pytest} & Limpo / limpo / 33 testes passando \\
...
\medskip
\noindent\textbf{Nota sobre escala da visualização.} A verificação usou 160
vetores. O gargalo prático da ferramenta não é o Milvus nem o PCA, e sim a
renderização no navegador: o Plotly desenha os pontos como SVG por padrão, o que
mantém a interação (rotação 3D, \emph{hover}, \emph{zoom}) fluida até a ordem de
alguns milhares de pontos e começa a degradar acima disso. Escalar para dezenas
de milhares exigiria trocar para renderização WebGL (\texttt{Scattergl}, que
suporta $\sim$100 mil pontos) ou amostrar/agregar a galeria antes de plotar.
Esses são limites esperados da abordagem de renderização, não medições deste
projeto, e ficam como trabalho futuro natural.

\section{Demonstração}

Esta seção mostra a interface em uso, capturada sobre a coleção de amostra
(\texttt{cbir\_sample}, 160 recortes de 4 classes de embarcações).

A Figura~\ref{fig:home} mostra a tela inicial: a galeria projetada em 2D por
PCA e colorida por classe, com o painel lateral de controles e a variância
explicada acumulada exibida no topo. A Figura~\ref{fig:controls} detalha os
controles: seleção de coleção (com o modelo de \emph{embedding} associado),
projeção 2D/3D, número de vizinhos $k$ e o voto ponderado por similaridade.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{images/screenshot_home.png}
  \caption{Tela inicial: projeção 2D da galeria, colorida por classe.}
  \label{fig:home}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[height=0.28\textheight]{images/screenshot_controls.png}
  \caption{Painel de controles, com a coleção e seu modelo de \emph{embedding}.}
  \label{fig:controls}
\end{figure}

Ao enviar uma imagem de consulta, a ferramenta a projeta no mesmo espaço
(losango vermelho), destaca os vizinhos recuperados (amarelo) e prevê a classe
por voto KNN. A Figura~\ref{fig:query2d} mostra o resultado em 2D: a consulta
(um recorte de \texttt{Traineira}) cai perto de outras traineiras, o painel de
predição reporta \emph{Traineira} com 70\% de confiança, e a faixa inferior
lista os dez vizinhos mais próximos com seus \emph{scores}. A
Figura~\ref{fig:query3d} mostra a mesma consulta na projeção 3D.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/screenshot_query_2d.png}
  \caption{Consulta em 2D: query destacada, vizinhos, predição KNN e a galeria
  dos dez vizinhos mais próximos com \emph{scores}.}
  \label{fig:query2d}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/screenshot_query_3d.png}
  \caption{A mesma consulta na projeção 3D.}
  \label{fig:query3d}
\end{figure}

A API expõe a documentação interativa gerada automaticamente pelo FastAPI
(Figura~\ref{fig:api}), com todos os \emph{endpoints} e os esquemas de dados
derivados dos modelos Pydantic.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{images/screenshot_api.png}
  \caption{Documentação interativa da API (FastAPI / OpenAPI).}
  \label{fig:api}
\end{figure}
