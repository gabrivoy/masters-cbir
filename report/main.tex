%=============================================================================
% Relatório Final — INF2102 Projeto Final de Programação — DI PUC-Rio
...
\title{CBIR — Explorador de Espaço Vetorial para Imagens de Embarcações}
...
\begin{center}
  ...
  {\huge\bfseries CBIR — Explorador de Espaço Vetorial\\ para Imagens de Embarcações}\\[0.4cm]
  \vspace*{1cm}
  {\large
    \vspace*{0.2cm}
    Gabriel Ribeiro \\[0.2cm]
    \vspace*{0.9cm}
    \emph{Orientador:} \\ {}[preencher]
  }
...
O \textbf{CBIR — Explorador de Espaço Vetorial} é uma ferramenta para
\emph{recuperação de imagens por conteúdo} (\emph{Content-Based Image
Retrieval}) aplicada a recortes de embarcações. O programa indexa
\emph{embeddings} de imagem em um banco de dados vetorial e permite
\textbf{explorar visualmente o espaço de representação}: projeta a galeria
indexada em 2D/3D via PCA, aceita uma imagem de consulta, e mostra onde ela se
posiciona em relação aos aglomerados (\emph{clusters}) de cada classe — além de
prever, por votação \emph{K-Nearest Neighbors} (KNN) sobre os vizinhos
