...
\begin{itemize}
  ...
  \item \textbf{OpenCLIP}: implementação aberta do modelo CLIP, que converte uma
  imagem em um vetor (\emph{embedding}) que captura seu conteúdo visual.
  \item \textbf{Milvus}: banco de dados vetorial, especializado em armazenar
  \emph{embeddings} e fazer busca por vizinhos mais próximos em larga escala.
  \item \textbf{scikit-learn (PCA)}: biblioteca de \emph{machine learning}; usamos
  o PCA para reduzir os vetores a 2D/3D preservando as direções de maior variância.
